my_stack* stack_create(int size_stack, int size_elem, int level_of_secur)
{
    ...
    tmp->cur_point = 0; //в байтах
    tmp->push_count = 0;
    tmp->pop_count = 0;
    tmp->errors = 0;
    tmp->size_of_stack = size_stack; //количество элементов
...
{
    //по задаче о блужданиях(берем отношение реального и теоретического значения размера) новый размер должен отличатся в корень из 2 тк я хочу в 2 раза его увеличить
...
{
    /*
    -n - траблы с хешом(выводится разница. чем она больше по модулю тем больше жопа со стеком)
    0 - все нормально
    1 - зачем мне null
    2 - есть восстановленные байты
    3 - капец байтам
    */
    if (_stack == NULL)
    {
